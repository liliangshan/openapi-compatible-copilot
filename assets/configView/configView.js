...
(function () {
	...
	const translations = {
		'en': {
			...
			chatHistorySavePath: 'Save Path',
			chatHistorySavePathHelp: "Directory to save chat history. Defaults to project's .LLSOAI folder.",
		...
		'zh-cn': {
			...
			chatHistorySavePath: '保存路径',
			chatHistorySavePathHelp: '保存聊天历史的目录。默认为项目的 .LLSOAI 文件夹。',
			expertMode: '专家模式',
	...
	function setupPanelEventListeners() {
		...
		panelSaveBtn?.addEventListener('click', () => {
			...
			if (mode === 'global') {
				...
				const projectSystemPrompt = document.getElementById('panelProjectSystemPrompt')?.value || '';
				const projectChatHistoryEnabled = document.getElementById('panelProjectChatHistoryEnabled')?.checked || false;
				const projectChatHistorySavePath = document.getElementById('panelProjectChatHistorySavePath')?.value.trim() || '';
	...
	window.addEventListener('message', (event) => {
		...
		switch (message.command) {
			...
			case 'projectChatHistorySettingsLoaded':
				if (message.data) {
					const panelProjectChatHistoryEnabled = document.getElementById('panelProjectChatHistoryEnabled');
					const panelProjectChatHistorySavePath = document.getElementById('panelProjectChatHistorySavePath');
					if (panelProjectChatHistoryEnabled) {
						panelProjectChatHistoryEnabled.checked = message.data.enabled;
					}
					if (panelProjectChatHistorySavePath) {
						panelProjectChatHistorySavePath.value = message.data.savePath || '';
					}
				}
				break;
	...
	function saveProjectSettings() {
		...
		vscode.postMessage({
			command: 'updateProjectChatHistorySettings',
			data: {
				enabled: document.getElementById('panelProjectChatHistoryEnabled')?.checked || false,
				savePath: document.getElementById('panelProjectChatHistorySavePath')?.value.trim() || ''
			}
		});
